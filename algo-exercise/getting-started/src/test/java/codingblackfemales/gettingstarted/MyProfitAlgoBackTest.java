...
public class MyProfitAlgoBackTest extends AbstractAlgoBackTest {
    ...
    public void backtestAlgoBehaviourFrom1stTickTo11thTick() throws Exception {
        ...
        send(createTick4());
            ...
            // get the last cancelled buy order in the list or else throw an assertion error if no such order is found  
            final ChildOrder lastCancelledBuyOrderPriceAfter4thTickIs98 = state.getChildOrders().stream()
                                                                               .filter(order -> order.getSide() == Side.BUY)
                                                                               .filter(order -> order.getState() == OrderState.CANCELLED)
                                                                               .reduce((first, second) -> second)
                                                                               .orElseThrow(() ->
                                                                                new AssertionError("Expected to find the last cancelled buy order in the list but found something else"));
        ...
        send(createTick5());
            ...
            // check that there is a new sell order in the list or else throw an assertion error if no such order is found                         
            final ChildOrder newSellOrderAfter5thTickIs102 = state.getChildOrders().stream()
                                                                  .filter(order -> order.getSide() == Side.SELL)
                                                                  .reduce((first, second) -> second)
                                                                  .orElseThrow(() -> 
                                                                   new AssertionError("Expected a new Sell order in the list but found something else"));
        ...
        send(createTick7());
            ...
            // get the last buy order from the list or else throw an assertion error if no such order is found                           
            final ChildOrder lastBoughtPriceAfter7thTickIs102 = state.getChildOrders().stream()
                                                                     .filter(order -> order.getSide() == Side.BUY)
                                                                     .reduce((first, second) -> second)
                                                                     .orElseThrow(() -> 
                                                                      new AssertionError("Expected to find the last buy order in the list but found something else"));
            // Get the last fully filled buy order after 7th tick                        
            final ChildOrder lastFilledBuyOrderPriceAfter7thTickIs102 = state.getChildOrders().stream()
                                                                             ...
                                                                             .orElseThrow(() -> 
                                                                              new AssertionError("Expected to find the last fully filled buy order but found something else"));
        ...
        send(createTick8());
            ...
            // get the last cancelled buy order or else throw an assertion error if no such order is found  
            final ChildOrder lastCancelledBuyOrderPriceAfter8thTickIs102 = state.getChildOrders().stream()
                                                                                .filter(order -> order.getSide() == Side.BUY)
                                                                                .filter(order -> order.getState() == OrderState.CANCELLED)
                                                                                .reduce((first, second) -> second)
                                                                                .orElseThrow(() -> 
                                                                                 new AssertionError("Expected to find the last cancelled buy order price but found something else"));                                                                              

            // get the last sell order or throw an assertion error if no such order is found  
            final ChildOrder newSellOrderPriceAfter8thTickIs105 = state.getChildOrders().stream()
                                                                       .filter(order -> order.getSide() == Side.SELL)
                                                                       .reduce((first, second) -> second)
                                                                       .orElseThrow(() ->
                                                                        new AssertionError("Expected to find the last sell order on the list but found something else"));

            // get the last sell order after 8th tick or throw an assertion error if this is not correct
            final ChildOrder newSellOrderedQtyAfter8thTickIs200 = state.getChildOrders().stream()
                                                                       .filter(order -> order.getSide() == Side.SELL)
                                                                       .reduce((first, second) -> second)
                                                                       .orElseThrow(() ->
                                                                        new AssertionError("Expected to find the last sell order but found something else"));  

            // get the last fullyl filled sell order or throw an assertion error if no order is found
            final ChildOrder newSellOrderFilledQtyAfter8thTickIs200 = state.getChildOrders().stream()
                                                                           .filter(order -> order.getSide() == Side.SELL)
                                                                           .reduce((first, second) -> second)
                                                                           .orElseThrow(() ->
                                                                            new AssertionError("Expected to find the last fully filled sell order but found something else"));

            // get the last cancelled sell order or throw an assertion error if no such order is found  
            final ChildOrder lastCancelledSellOrderPriceAfter8thTickIs105 = state.getChildOrders().stream()
                                                                                 .filter(order -> order.getSide() == Side.SELL)
                                                                                 .filter(order -> order.getState() == OrderState.CANCELLED)
                                                                                 .reduce((first, second) -> second)
                                                                                 .orElseThrow(() -> 
                                                                                  new AssertionError("Expected to get the last cancelled sell order but found something else"));
        ...
        send(createTick9());
            ...
            // get the last buy order on the list or throw an assertion error if no such order is found                            
        ...
        send(createTick10());
            ...
            // get last buy order after 10th tick or throw an assertion error if no such order is found                            
        ...
        //     final String updatedOrderBook11 = Util.orderBookToString(container.getState());  
            ...
            // // assertEquals(9, priceReversalThresholdAt11thTick); // 4/11/2024 - expected 9 but was 8
